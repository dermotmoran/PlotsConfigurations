...
void AutoRebin::FindNewBinning(TH1F &total_bkg, std::vector<double> &new_bins, 
  ...
  //minimises the number of required bin merges.     
    ...
    if(bin_tot_flag || bin_err_flag) {
	  ...
	  left_err_tot/left_tot >= bin_uncert_fraction)) && idx > 0 && int(total_bkg.GetBinLowEdge(idx+1))%10!=0) { //DM
	  ...
	  right_err_tot/right_tot >= bin_uncert_fraction)) && idx <= nbins  && int(total_bkg.GetBinLowEdge(idx))%10!=0){ //DM
