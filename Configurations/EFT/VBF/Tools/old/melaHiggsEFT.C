...
std::vector<float> melaHiggsEFT(Mela *_mela, TVar::MatrixElement ME, TVar::Production Prod, bool IsGG, bool IsReco){
  ...
  float g    = -13752.22;
  float intf = me_mixhl - me_hsm - me_hl;
  float me   = intf*1*g + me_hsm*pow(1,2) + me_hl*pow(g,2);
  ...
  _mela->setProcess(TVar::SelfDefine_spin0, ME, Prod);
  _mela->selfDHzzcoupl[0][gHIGGS_VV_1][0]= 1.;
  _mela->selfDHzzcoupl[0][gHIGGS_VV_1_PRIME2][0]= g;   
  if(IsGG)  _mela->selfDHggcoupl[0][gHIGGS_GG_2][0]=1;                         
  if(Decay) _mela->computeP(me2, IsReco);
  else      _mela->computeProdP(me2, IsReco); 

  std::cout<<" IsGG/Decay "<< IsGG <<"/"<< Decay <<" "<< me2/me <<" "<<std::endl;
  std::cout<<" "<<std::endl;
